...
class TikTokFinanceProcessor {
  ...
  calculateTotalPlatformCosts(receivedOrders) {
    ...
    // Debug: Log orderProcessingFee
    const orderProcessingFeeOrders = receivedOrders.filter(order => parseFloat(order.orderProcessingFee) !== 0);
    console.log(`[TikTokFinanceProcessor] OrderProcessingFee: ${orderProcessingFeeOrders.length} orders with fee, total: ${costBreakdown.orderProcessingFee.toLocaleString()}`);
    
    // Debug: Log shipping fees
    const shippingFeeOrders = receivedOrders.filter(order => parseFloat(order.shippingFee) !== 0);
    const shopShippingFeeOrders = receivedOrders.filter(order => parseFloat(order.shopShippingFee) !== 0);
    console.log(`[TikTokFinanceProcessor] ShippingFee: ${shippingFeeOrders.length} orders, total: ${costBreakdown.shippingFee.toLocaleString()}`);
    console.log(`[TikTokFinanceProcessor] ShopShippingFee: ${shopShippingFeeOrders.length} orders, total: ${costBreakdown.shopShippingFee.toLocaleString()}`);
